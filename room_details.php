...
<body class="bg-light">
    ...
    // Giả sử bạn đã có hàm filteration() đã xử lý dữ liệu GET
    $data = filteration($_GET);
    
    include("connect.inp");
    
    // Sử dụng Prepared Statement để tránh lỗi và SQL Injection
    $stmt = $con->prepare("SELECT * FROM `rooms` WHERE room_id=? AND status = 'available'");
    $stmt->bind_param('i', $data['id']);  // 'i' chỉ ra rằng tham số là kiểu integer
    $stmt->execute();
